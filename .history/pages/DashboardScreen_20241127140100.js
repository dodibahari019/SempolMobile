...
export default function App({ navigation }) {
  ...
  const dataDiagramLingkaran = [
   ...
   const chartSize = width * 0.7; // Grafik 40% dari lebar layar (lebih kecil)
  const radius = chartSize / 2; // Radius lingkaran (1/4 dari ukuran chart)
  const circumference = 2 * Math.PI * radius; // Keliling lingkaran
  const data = [
    { percentage: 40, color: '#f00', label: 'Red' },
    { percentage: 30, color: '#0f0', label: 'Green' },
    { percentage: 30, color: '#00f', label: 'Blue' },
  ];
...
//Date Picker
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1 }} style={{ backgroundColor:'#d9d9d9', marginTop:20}}>
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1 }} style={{ backgroundColor:'#e8e7e7', borderRadius:30, bottom:0, display:'flex', marginTop:3 }}>
                ...
                {isVisible == 'Beranda' && (
                  <ScrollView style={{ borderWidth:0.5, display:'flex', flex:1, borderColor:'#afaeae', margin:10, padding:20, borderRadius:10, }}>
                      ...
                      <TouchableOpacity>
                          <TextInput style={{ fontFamily:'HowdyLemon', marginTop:30, borderWidth:0.5, borderColor:'#afaeae', borderRadius: 10, color:'#545454', padding:10}} value="Semua Pemasukan & Pengeluaran" editable={false} />
                      </TouchableOpacity>
                      <View style={{ display:'flex', flex:1, height:height * 0.35, marginTop:8, borderWidth:0.5, borderColor:'#afaeae', padding:10, borderRadius:10, justifyContent:'center', alignItems:'center'}}>
                       <View
        style={{
          position: 'relative',
          width: chartSize,
      ...
      >
        <Svg height={chartSize} width={chartSize} viewBox={`0 0 ${radius * 2} ${radius * 2}`}>
          {data.map((item, index) => {
            ...
            return (
              <Circle
                ...
                fill="transparent"
                stroke={item.color}
                strokeWidth={radius * 0.15} // Ketebalan stroke dinamis (15% dari radius)
        ...
        <View
          style={{
            ...
            top: '50%',
            left: '50%',
            transform: [{ translateX: -chartSize * 0.1 }, { translateY: -chartSize * 0.08 }],
            alignItems: 'center',
          }}
        >
          <Text style={{ fontSize: chartSize * 0.05, color: '#333', fontWeight: 'bold' }}>
            Total
          </Text>
          <Text style={{ fontSize: chartSize * 0.04, color: '#555' }}>Rp1.491.000</Text>
        </View>
      </View>
      ...
      >
        {data.map((item, index) => (
          ...
          >
            <View
              style={{
                width: chartSize * 0.04,
                height: chartSize * 0.04,
                borderRadius: chartSize * 0.02,
                backgroundColor: item.color,
                marginRight: 5,
              }}
            />
            <Text style={{ fontSize: chartSize * 0.035, color: '#333' }}>{item.label}</Text>
          </View>
        ))}
      </View>
                      </View>
