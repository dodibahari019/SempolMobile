...
export default function App({ navigation }) {
  ...
  const data = [
    { month: "Jan", totalSales: 500, avgSales: 400 },
    { month: "Feb", totalSales: 700, avgSales: 500 },
    { month: "Mar", totalSales: 600, avgSales: 450 },
    { month: "Apr", totalSales: 900, avgSales: 600 },
  ];

  const chartWidth = 300; // Lebar grafik
  const chartHeight = 200; // Tinggi grafik
  const barWidth = 30; // Lebar batang
  const maxValue = Math.max(...data.map((item) => item.totalSales)); // Nilai maksimal untuk skala
  const xGap = chartWidth / data.length;

  const fetchData = async () => {
    try {
        const response = await axios.get('http://192.168.28.215/SempolMobileApp/sempolAPI/getDataDashboard.php');
...
const handleSubmit = () => {
  ...
  // Contoh POST ke API
  fetch('http://192.168.28.215/SempolMobileApp/sempolAPI/insertDataBaru.php', {
    method: 'POST',
    headers: {
...
//Date Picker
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1 }} style={{ backgroundColor:'#d9d9d9', marginTop:20}}>
        ...
        {data.map((item, index) => {
          const barHeight = (item.totalSales / maxValue) * chartHeight; // Skala tinggi batang
          return (
            <Rect
              key={`bar-${index}`}
              x={index * xGap + barWidth / 2}
        ...
        {data.map((item, index) => {
          if (index === data.length - 1) return null; // Hindari data terakhir
          const x1 = index * xGap + xGap / 2;
          const y1 =
            chartHeight - (data[index].avgSales / maxValue) * chartHeight;
          const x2 = (index + 1) * xGap + xGap / 2;
          const y2 =
            chartHeight - (data[index + 1].avgSales / maxValue) * chartHeight;
        ...
        {data.map((item, index) => {
          ...
          const cy =
            chartHeight - (item.avgSales / maxValue) * chartHeight;
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 10, width: chartWidth }}>
        {data.map((item, index) => (
          <Text key={`label-${index}`} style={{ fontSize: 12, textAlign: "center", flex: 1 }}>
            {item.month}
          </Text>
        ))}
      </View>
