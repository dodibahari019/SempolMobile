...
export default function App({ navigation }) {
  const { isVisible, activeButton, subMenu, activeSubmenuButton, isModalVisible, isModalVisibleChartOne, currentDate} = useContext(VariableDash);
  const [, setModalVisible] = useState(false);
  const [, setModalVisibleChartOne] = useState(false);
  const [, setCurrentDate] = useState('');
  const [, setDate] = useState(new Date());  // State untuk menyimpan tanggal
