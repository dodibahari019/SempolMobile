...
export default function App({ navigation }) {
  ...
  const [fontsLoaded] = useFonts({
   ...
   const dataCabang = [
    { branch: "Jakarta", sales: 500 },
    { branch: "Bandung", sales: 300 },
    { branch: "Surabaya", sales: 400 },
    { branch: "Medan", sales: 250 },
  ];

  const chartWidthCab = 300;
  const chartHeightCab = 200;
  const barHeight = 20;
  const maxValueCab = Math.max(...data.map((item) => item.sales));
