...
export default function App({ navigation }) {
  ...
  const [plastikSaos, setPlastikSaos] = useState('');
   ...
   const data = [
    { name: "Cash", population: 60, color: "blue", legendFontColor: "#7F7F7F", legendFontSize: 12 },
    { name: "GoFood", population: 30, color: "green", legendFontColor: "#7F7F7F", legendFontSize: 12 },
    { name: "QRIS", population: 10, color: "red", legendFontColor: "#7F7F7F", legendFontSize: 12 },
  ];
  // Muat font custom
...
//Date Picker
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1 }} style={{ backgroundColor:'#d9d9d9', marginTop:20}}>
        ...
        style={{ height: 200 }}
        data={[
          { value: 40, color: "#4caf50" }, // Cash
          { value: 30, color: "#2196f3" }, // GoFood
          { value: 30, color: "#ff9800" }, // QRIS
        ]}
        innerRadius={50} // Membuat hole di tengah (doughnut)
        outerRadius={100} // Radius luar
      />
                      </View>
                  </ScrollView>
                )}
