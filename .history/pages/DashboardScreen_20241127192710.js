...
export default function App({ navigation }) {
  ...
  const barHeight = 20;
  const maxValueCab = Math.max(...dataCabang.map((item) => item.sales));
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1 }} style={{ backgroundColor:'#e8e7e7', borderRadius:30, bottom:0, display:'flex', marginTop:3 }}>
                ...
                {isVisible == 'Beranda' && (
                  <ScrollView style={{ borderWidth:0.5, display:'flex', flex:1, borderColor:'#afaeae', margin:10, padding:20, borderRadius:10, }}>
                      ...
                      {/* GRAFIK CABANG */}
                      <TouchableOpacity style={{ marginTop:30, borderWidth:0.5, borderColor:'#afaeae', borderRadius: 10, color:'#545454', padding:6, justifyContent:'center',}}>
                        <Text style={{fontFamily:'HowdyLemon', color:'#545454', padding:5,}}>Cabang dengan Penjualan Tertinggi</Text>
                      </TouchableOpacity>
                      <View style={{ display:'flex', flex:1, marginTop:8, borderWidth:0.5, borderColor:'#afaeae', padding:10, borderRadius:10, justifyContent:'center', alignItems:'center'}}>
                       <View style={{ position: "relative", width: '100%', justifyContent:'center', alignItems:'center', padding:10,}}>
                         <Svg height={chartHeightCab} width={chartWidthCab}>
                            {dataCabang.map((item, index) => {
                              const barWidthCab = (item.sales / maxValueCab) * chartWidthCab;
