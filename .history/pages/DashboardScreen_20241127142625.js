...
export default function App({ navigation }) {
  ...
  const [plastikSaos, setPlastikSaos] = useState('');const chartSize = width * 0.4; // Grafik 40% dari lebar layar (lebih kecil)
  const radius = chartSize / 4; // Radius lingkaran (1/4 dari ukuran chart)
  const circumference = 2 * Math.PI * radius; // Keliling lingkaran
  const data = [
    { percentage: 40, color: '#f39c12', label: 'Cash' },
    { percentage: 30, color: '#ff7675', label: 'GoFood' },
    { percentage: 30, color: '#9b59b6', label: 'QRIS' },
  ];
